...
def main():
    ...
    if opt.config != 'configs/stable-diffusion/v1-inference.yaml':
        config.model.params.cond_stage_config.params['extend_outputlen']=1232
        config.model.params.cond_stage_config.params['max_length']=1232
        max_src_length = 1232 ## 1232
    ...
    anno_file = json.load(open(opt.from_file,'r'))
    caption_dict = {}
    caption_anno_train = json.load(open('dataset/captions_train2017.json','r'))
    caption_anno_val = json.load(open('dataset/captions_val2017.json','r'))
    for sample in caption_anno_train['annotations']:
        if sample['image_id'] in caption_dict:
            caption_dict[sample['image_id']].append(sample['caption'])
        else:
            caption_dict[sample['image_id']] = [sample['caption']]
    for sample in caption_anno_val['annotations']:
        if sample['image_id'] in caption_dict:
            caption_dict[sample['image_id']].append(sample['caption'])
        else:
            caption_dict[sample['image_id']] = [sample['caption']]

    val17only = True ## of 19809 images, 9272 are in val14, others in train 14; so use val17, where all 4809 are from val14
    data, coco_od = [], {}
    # counter=0
    for img_ii in range(len(anno_file['images'])):
        sample = anno_file['images'][img_ii]
        if val17only and 'train2017' in sample['coco_url']:
            continue
        data.append([str(sample['id']),caption_dict[sample['id']][0].replace('/',' '), (sample['height'], sample['width'])])
    ...
    coco_od = json.load(open('dataset/lvis_allbox_gitcaption.json', 'r')) ## 'tag' 'pad_caption' 'crop_caption'
    num_bins, centercrop = 1000, opt.centercrop
    num_withbox = 0
    for key in coco_od:
        if 'box' in coco_od[key]:
            if len(coco_od[key]['box'])!=0: num_withbox+=1
    # print('num_withbox',num_withbox)
            ...
            with model.ema_scope():
                ...
                for n in trange(opt.n_iter, desc="Sampling"):
                    ...
                    for sample in tqdm(data, desc="data"):
                        ...
                        str_caption, str_caption_full = text[0], text[0]
                            ...
                            if 'box' in coco_od[sample_idx]:   ## Use OD box instead
                                ...
                                for sample_ii in range(number_box):
                                    ...
                                    if opt.box_descp=='tag':
                                        caption = box_sample['tag'].replace('(','').replace(')','')
                                    elif 'caption' in opt.box_descp: ## 'capton' 'cococaption'
                                        # caption = 'a photo of a %s'%box_sample['tag'].replace('(','').replace(')','')
                                        ...
                                        if random.random()<0.5:
                                            ...
                                            caption = box_sample['crop_caption'] if '"' not in box_sample['crop_caption'] else box_sample['pad_caption']
                                    box_list.append((box,caption,box_sample['tag'].replace('(','').replace(')',''))) ## include tag for visu
                        if len(box_list)!=0:
                            ...
                            for box_sample in box_list:
                                if opt.spatial_word is None:
                                    ...
                                    str_caption_full += ' <%d> <%d> <%d> <%d> '%(quant_x0-box_offset,quant_y0-box_offset,quant_x1-box_offset,quant_y1-box_offset) + caption
                                else:
                                    box, caption = box_sample[0], box_sample[1]
                                    box = process_centercrop(box, imagehw, centercrop)
                                    if box[0]==1.0 or box[1]==1.0 or box[2]==0.0 or box[3]==0.0:
                        ...
                        # if opt.box_descp=='caption' and max_src_length==77:
                        #     pad_tokenized_text = torch.tensor([box_offset-1]*max_src_length*5).to(text_enc.device)    ## manual set extended pos emd length
                        # else:
                        ...
                        prompt_list.append(str(sample_idx)+'\t'+str_caption_full)
                        ...
                        img_path = opt.cocogt + '/%012d.jpg'%(int(sample_idx))
    ...
    print(f"Your samples are ready and waiting for you here: \n{outpath} \n"
          f" \nEnjoy.")
    with open(os.path.join(outpath, '../lvis_prompts_%d.txt'%local_rank), 'w') as f:
